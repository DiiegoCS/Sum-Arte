...
def generar_reporte_estado_proyecto_pdf(proyecto):
    ...
    Raises:
        Exception: Si hay un error al generar el PDF
    """
    try:
        logger.info(f"Iniciando generación de PDF para proyecto {proyecto.id}")
        ...
        nombre_organizacion = proyecto.id_organizacion.nombre_organizacion if proyecto.id_organizacion else 'N/A'
        estado_display = proyecto.get_estado_proyecto_display() if hasattr(proyecto, 'get_estado_proyecto_display') else proyecto.estado_proyecto
        
        info_data = [
            ['Proyecto:', proyecto.nombre_proyecto or 'N/A'],
            ['Organización:', nombre_organizacion],
            ['Estado:', estado_display],
            ['Fecha de inicio:', proyecto.fecha_inicio_proyecto.strftime('%d/%m/%Y') if proyecto.fecha_inicio_proyecto else 'N/A'],
            ['Fecha de fin:', proyecto.fecha_fin_proyecto.strftime('%d/%m/%Y') if proyecto.fecha_fin_proyecto else 'N/A'],
        ]
        ...
        items = Item_Presupuestario.objects.filter(proyecto=proyecto).prefetch_related('subitems')
        ...
        # Construir PDF
        logger.info(f"Construyendo PDF para proyecto {proyecto.id}")
        doc.build(elements)
        buffer.seek(0)
        logger.info(f"PDF generado exitosamente para proyecto {proyecto.id}")
        return buffer
    except Exception as e:
        logger.error(f"Error al generar PDF para proyecto {proyecto.id}: {str(e)}", exc_info=True)
...
def generar_reporte_rendicion_oficial_pdf(proyecto):
    ...
    """
    try:
        logger.info(f"Iniciando generación de reporte oficial para proyecto {proyecto.id}")
        ...
        # Dividir el texto de certificación en múltiples Paragraphs para evitar problemas de parsing
        nombre_proyecto_escaped = proyecto.nombre_proyecto or 'N/A'
        fecha_cierre_str = fecha_cierre.strftime('%d de %B de %Y a las %H:%M:%S')
        
        elements.append(Paragraph(
            f'Por medio del presente documento, se certifica que la rendición del proyecto '
            f'<b>"{nombre_proyecto_escaped}"</b> ha sido cerrada y validada según los controles '
            f'establecidos en el sistema Sum-Arte.',
            styles['Normal']
        ))
        elements.append(Spacer(1, 0.2*inch))
        
        elements.append(Paragraph('Se confirma que:', styles['Normal']))
        ...
        elements.append(Paragraph(
            f'Este reporte fue generado automáticamente el {fecha_cierre_str} '
            f'y constituye el documento oficial de rendición del proyecto.',
            styles['Normal']
        ))
        elements.append(Spacer(1, 0.5*inch))
        ...
        # Construir PDF
        logger.info(f"Construyendo reporte oficial para proyecto {proyecto.id}")
        doc.build(elements)
        buffer.seek(0)
        logger.info(f"Reporte oficial generado exitosamente para proyecto {proyecto.id}")
        return buffer
    except Exception as e:
        logger.error(f"Error al generar reporte oficial para proyecto {proyecto.id}: {str(e)}", exc_info=True)
        raise Exception(f"Error al generar el reporte oficial de rendición: {str(e)}")
